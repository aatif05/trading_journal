...
function findSwingPoints(highArr: number[], lowArr: number[], window = 3): SwingPoint[] {
  const points: SwingPoint[] = [];
  for (let i = window; i < highArr.length - window; i++) {
    const highWindow = highArr.slice(i - window, i + window + 1);
    if (highArr[i] === Math.max(...highWindow)) points.push({ index: i, type: "high", price: highArr[i] });
    const lowWindow = lowArr.slice(i - window, i + window + 1);
    if (lowArr[i] === Math.min(...lowWindow)) points.push({ index: i, type: "low", price: lowArr[i] });
  }
  ...
  for (const point of points) {
    ...
    if (!last) {
      ...
      const keepNew = point.type === "high" ? point.price > last.price : point.price < last.price;
      if (keepNew) alternating[alternating.length - 1] = point;
    } else {
      alternating.push(point);
    }
  }
...
type Contraction = { high: number; low: number; depthPct: number; avgVolume: number };

function buildContractions(points: SwingPoint[], volumeSlice: number[]): Contraction[] {
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    if (a.type === "high" && b.type === "low") {
      ...
      const avgVolume = segment.length ? segment.reduce((sum, val) => sum + val, 0) / segment.length : 0;
      contractions.push({ high: a.price, low: b.price, depthPct: (a.price - b.price) / a.price, avgVolume });
    }
  }
  return contractions;
}
 ...
 */
function detectVCP(ticks: PriceTick[]): { pivot: number; confidence: "Low" | "Medium" | "High"; evidence: string[] } | null {
  const h = highs(ticks);
  const l = lows(ticks);
  ...
  const LOOKBACK = 90;
  const start = Math.max(0, h.length - LOOKBACK);
  const points = findSwingPoints(h.slice(start), l.slice(start), 3);
  const contractions = buildContractions(points, v.slice(start));

  const last = contractions.slice(-4);
  if (last.length < 2) return null;

  const validSequence = last.every(
    (leg, i) => i === 0 || (leg.depthPct <= last[i - 1].depthPct * 1.15 && leg.avgVolume <= last[i - 1].avgVolume * 1.15),
  );
  ...
  const volumeDry = avgVolPrior > 0 && avgVolRecent < avgVolPrior * 0.8;

  const confidence: "Low" | "Medium" | "High" =
    last.length >= 3 && nearPivot && volumeDry ? "High" : nearPivot || volumeDry ? "Medium" : "Low";
  ...
  return {
    ...
    evidence: [
      ...
      volumeDry ? "Volume has dried up into the pivot" : "Volume not yet confirming dry-up",
      nearPivot ? "Price is near the pivot high" : "Price remains below the pivot",
    ],
  };
}
    ...
    if (brokeOut || nearBox) {
      result.push({
        ...
        evidence: [
          ...
          brokeOut ? "Price has broken above the box top" : "Price is approaching the box top, not yet confirmed",
