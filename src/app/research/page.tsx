  ...
  return (
    <main className="min-h-screen bg-[#f7f9f7] pb-28 text-[#202923]">
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-8">
        ...
        {/* Pattern radar — now full width so mini charts get real room to breathe */}
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {marketSeries.length ? (
              marketSeries.map((market) => {
                ...
                const values = market.ticks.map((tick) => Number(tick[4])).filter((value) => Number.isFinite(value));
                const recent = values.slice(-20);
                const typedTicks = market.ticks as [string, string, string, string, string, string][];

                if (item) {
                  const ceiling = Math.max(...market.ticks.map((tick) => Number(tick[2])));
                  const floor =
                    item.pattern === "Darvas Box" && item.stopLevel != null
                      ? item.stopLevel
                      : Math.min(...market.ticks.map((tick) => Number(tick[3])));
                    ...
                    <div key={market.symbol} className="rounded-xl bg-[#f7f9f7] p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-[#7b867f]">
                            1Y context · recent 90 focused · trigger{" "}
                  ...
                  <div key={market.symbol} className="rounded-xl bg-[#f7f9f7] p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-[#7b867f]">
                          No confirmed VCP or Darvas setup yet · {market.ticks.length} sessions
                        </p>
                      </div>
                    ...
                    <div className="mt-3 h-48">
                      <PatternMiniChart
                        ticks={typedTicks}
                        ceiling={Math.max(...recent)}
                        floor={Math.min(...recent)}
                        breakout={Math.max(...recent)}
                      />
                    </div>
                  </div>
                );
