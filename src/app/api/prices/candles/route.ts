...
export async function GET(request: NextRequest) {
  ...
  const requestedLookback = range === "1M" ? 60 * 24 * 31 : range === "3M" ? 60 * 24 * 92 : range === "1Y" ? 60 * 24 * 365 : range === "2Y" ? 60 * 24 * 730 : 60 * 24 * 183;
  const endParam = request.nextUrl.searchParams.get("end");
  const endDate = endParam ? new Date(endParam) : undefined;
  const lookback = endDate ? Math.min(requestedLookback, 60 * 24 * 365) : requestedLookback;
  const candleInterval = interval === "1m" ? "1m" : interval === "5m" ? "5m" : interval === "15m" ? "15m" : interval === "1h" ? "1h" : "1d";
  try {
    const ticks = await fetchStrikeTicks(symbol, lookback, candleInterval, fetch, endDate);
