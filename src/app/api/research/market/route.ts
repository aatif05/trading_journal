...
export async function GET(request: Request) {
  ...
  if (!symbols.length) return NextResponse.json({ series: [], patterns: [], themes: [] });
  const series = (await Promise.all(symbols.map(async (symbol) => ({ symbol, ticks: await fetchStrikeTicks(symbol, 60 * 24 * 45, "1d") })))).filter((item) => item.ticks.length);
