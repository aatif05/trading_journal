...
export function useCapitalFlows() {
  const { data, error, mutate } = useSWR<{ flows: CapitalFlows; trades?: unknown[]; [key: string]: unknown }>("/api/journal", fetcher, { revalidateOnFocus: false });
  ...
  const updateFlow = useCallback((year: number, month: number, field: keyof CapitalFlow, value: number) => {
    ...
    void mutate((current) => ({ ...(current ?? {}), flows: next }), false);
