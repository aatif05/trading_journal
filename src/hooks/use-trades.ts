...
export function useTrades() {
  ...
  const addTrade = useCallback(() => { const next = createTrade(trades); void persist([...trades, next]); return next; }, [persist, trades]);
  const updateTrade = useCallback((id: string, patch: Partial<Trade>) => { void persist(trades.map((trade) => trade.id === id ? { ...trade, ...patch } : trade)); }, [persist, trades]);
